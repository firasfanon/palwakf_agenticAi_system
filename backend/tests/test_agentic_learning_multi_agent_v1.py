...
SOURCE_SHA = "19b9c8e9d2c52b955dfb1fd273dfecaa4f8383e6"
...
def _fixture():
    root = _root()
    agent = next(a for a in build_projection(root, SOURCE_SHA) if a.runnable)
    ...
    env = ExecutionEnvironment(project_id=PROJECT_ID, repository="firasfanon/palwakf_agenticAi_system", task_branch=BRANCH, base_sha=BASE_SHA, expected_head=SOURCE_SHA, worktree=str(root), filesystem_policy=FilesystemPolicy(mode="READ_ONLY", allowed_roots=[str(root)]), network_policy=NetworkPolicy(read=True, write=False), tool_policy=[])
    package = WorkspaceStatePackage(state_package_id="state-b-e2e", project_id=PROJECT_ID, task_id=TASK_ID, repository="firasfanon/palwakf_agenticAi_system", task_branch=BRANCH, base_sha=BASE_SHA, expected_head=SOURCE_SHA, authorization=auth, authority_source="WORKSPACE_MANAGER")
...
def test_execute_and_learn_under_external_authority():
    ...
    result = AgenticLearningService(project_root=root, source_commit_sha=SOURCE_SHA).execute_and_learn(package=package, request=request)
...
def test_remote_wip_head_may_advance_beyond_historical_base():
    _, package, request = _fixture()
    assert package.base_sha == BASE_SHA
    assert package.expected_head == SOURCE_SHA
    ExternalContractAdapter().validate_workspace_package(package)
    ExternalContractAdapter().validate_run_binding(package=package, request=request)


def test_external_adapter_rejects_package_run_expected_head_mismatch():
    _, package, request = _fixture()
    bad_environment = request.environment.model_copy(update={"expected_head": "0" * 40})
    bad_request = request.model_copy(update={"environment": bad_environment})
    with pytest.raises(ValueError, match="RUN_EXPECTED_HEAD_BINDING_MISMATCH"):
        ExternalContractAdapter().validate_run_binding(package=package, request=bad_request)


def test_runtime_rejects_stale_expected_head_even_when_package_and_request_match():
    root, package, request = _fixture()
    stale = "0" * 40
    stale_package = package.model_copy(update={"expected_head": stale})
    stale_environment = request.environment.model_copy(update={"expected_head": stale})
    stale_request = request.model_copy(update={"environment": stale_environment})
    ExternalContractAdapter().validate_run_binding(package=stale_package, request=stale_request)
    from palwakf_local_agents.agentic_core_v1.runtime import AuthorityError
    with pytest.raises(AuthorityError, match="SOURCE_SHA_MISMATCH"):
        AgenticLearningService(project_root=root, source_commit_sha=SOURCE_SHA).runtime.execute(stale_request)
