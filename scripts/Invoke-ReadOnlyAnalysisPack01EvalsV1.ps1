...
$expected = @(
  @{
    ...
    task_file = 'PACK01_COORDINATOR_ROUTING_PILOT_001.json'
    profile_file = 'coordinator.json'
  },
  @{
    agent_id = 'sovereignty_reviewer'
    task_file = 'PACK01_SOVEREIGNTY_REVIEW_PILOT_001.json'
    profile_file = 'sovereignty_reviewer.json'
  },
  @{
    agent_id = 'knowledge_researcher'
    task_file = 'PACK01_KNOWLEDGE_RESEARCH_PILOT_001.json'
    profile_file = 'knowledge_researcher.json'
  },
  @{
    agent_id = 'documentation_handoff'
    task_file = 'PACK01_DOCUMENTATION_HANDOFF_PILOT_001.json'
    profile_file = 'documentation_handoff.json'
  }
)
...
foreach ($item in $expected) {
  $taskPath = Join-Path $templateRoot $item.task_file
  $profilePath = Join-Path $profileRoot $item.profile_file

  $task = Get-Content -LiteralPath $taskPath -Raw -Encoding UTF8 | ConvertFrom-Json
  $profile = Get-Content -LiteralPath $profilePath -Raw -Encoding UTF8 | ConvertFrom-Json

  $passed = (
    $task.requested_agent -eq $item.agent_id -and
    $task.status -eq 'PENDING_HUMAN_APPROVAL' -and
    $task.risk -eq 'LOW' -and
    $task.autonomy -eq 'L0_READ_ONLY' -and
    @($task.requested_skills).Count -eq 2 -and
    ...
    $profile.agent_id -eq $item.agent_id -and
  ...
  $results += [PSCustomObject]@{
    case_id = "PACK01_TEMPLATE_PROFILE_$($item.agent_id)"
    passed = $passed
  }
}

$uniqueTaskIds = @($taskIds | Select-Object -Unique)
$uniquePass = ($uniqueTaskIds.Count -eq $taskIds.Count)
...
$reportPath = Join-Path $outputDirectory "READ_ONLY_ANALYSIS_PACK_01_EVAL_REPORT_$stamp.json"
